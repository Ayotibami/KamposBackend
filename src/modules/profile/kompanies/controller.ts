...
export const create = async (req: Request, res: Response) => {
  ...
  const { avitag, display_name, email, phone_number, website, social_links, description, image_url } = req.body || {};
  if (!avitag || !display_name || !email || !phone_number || !website) {
    return res.status(400).json({ success: false, message: 'avitag, display_name, email, phone_number, website are required' });
  }
  ...
  void sendWelcomeEmail(req.user.account_id, { profile_type: 'KOMPANY', display_name });
