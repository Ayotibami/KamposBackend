...
export const EventCommentController = {
  create: async (req: Request, res: Response) => {
    ...
    if (!event_id || !text) return res.status(400).json({ success: false, message: 'event_id and text are required' });
    const created = await Repo.create({ event_id, avitag, text });
    try { WSGateway.broadcast('event_comment:created', { event_id, comment: created }); } catch {}
  ...
  update: async (req: Request, res: Response) => {
    ...
    try { WSGateway.broadcast('event_comment:updated', { event_id: updated.event_id, comment: updated }); } catch {}
