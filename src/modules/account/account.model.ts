...
export const createAccount = async (
  ...
  const { rows } = await pool.query(
    ...
    [
      a.email,
      a.passwordHash ?? null,
      a.authProvider ?? "Email",
      a.isOtpVerified ?? false,
...
export const findAccountByOauthId = async (
  oauthId: string
): Promise<IAccount | null> => {
  const { rows } = await pool.query(
    `SELECT * FROM accounts WHERE oauth_id = $1 LIMIT 1`,
    [oauthId]
  );
  if (!rows[0]) return null;
  return mapRow(rows[0]);
...
export const updateAccountById = async (
  ...
  vals.push(id);
  const { rows } = await pool.query(
    `UPDATE accounts SET ${set.join(
      ", "
    )}, updated_at = now() WHERE account_id = $${idx} RETURNING *`,
    vals
  );
  if (!rows[0]) return null;
  return mapRow(rows[0]);
