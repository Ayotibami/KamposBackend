...
export const OAuthService = {
  googleLogin: async (params: { id_token: string; refresh_token?: string | null; refresh_expires_at?: Date | string | null; }) => {
  ...
  facebookLogin: async (params: { access_token: string; refresh_token?: string | null; refresh_expires_at?: Date | string | null; }) => {
  ...
  appleLogin: async (params: { identity_token: string; refresh_token?: string | null; refresh_expires_at?: Date | string | null; }) => {
    ...
    const email: string | null = (payload.email as string | undefined) ?? null;
...
async function getAppleJWKS(): Promise<{ keys: any[] }> {
  const now = Date.now();
...
function decodeJwtHeader(token: string): JwtHeader & { kid: string } {
  const [h] = token.split('.');
...
function base64ToBufferUrl(b64url: string): Buffer {
  const b64 = b64url.replace(/-/g, '+').replace(/_/g, '/');
  return Buffer.from(b64, 'base64');
}

function rsaPublicKeyPem(modulusB64Url: string, exponentB64Url: string): string {
