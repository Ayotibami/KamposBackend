...
export function setAuthCookies(res: Response, accessToken: string, refreshToken: string) {
  res.cookie(ACCESS_COOKIE, accessToken, { ...baseOptions, maxAge: env.ACCESS_TOKEN_EXPIRES * 1000 });
  res.cookie(REFRESH_COOKIE, refreshToken, { ...baseOptions, maxAge: REFRESH_TOKEN_EXPIRES_SECONDS * 1000 });
}
