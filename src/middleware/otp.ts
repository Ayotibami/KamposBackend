...
export async function requireOtpVerified(req: Request, res: Response, next: NextFunction) {
  try {
    const user = req.user;
    if (!user?.account_id) {
    ...
    // Quick token hint
    if (user.is_otp_verified) return next();

    // Fetch latest account state
    const account = await accountRepo.findAccountById(user.account_id);
    if (account?.is_otp_verified) return next();
