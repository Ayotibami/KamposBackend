...
export const root = {
    ...
    counts: async ({ gist_id }) => GistService.getCounts(gist_id),
    ...
    // Subscriptions
    broadcast: ({ topic }) => {
        const iter = PubSub.asyncIterator(`broadcast:${topic}`);
...
// GraphQL (feeds only)
app.use('/graphql', fakeAuth, graphqlHTTP({ schema, rootValue: root, graphiql: env.NODE_ENV !== 'production' }));
