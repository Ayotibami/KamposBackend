...
export const EventController = {
    create: async (req, res) => {
        ...
        if (thumb && thumb.data) {
            ...
            const uploaded = await uploadBuffer(buffer, `kampos/events`);
        ...
        try {
            WSGateway.broadcast('event.created', { event: ev });
        }
        catch { }
    ...
    update: async (req, res) => {
        ...
        if (!updated)
            return res.status(404).json({ success: false, message: 'Event not found' });
        try {
            WSGateway.broadcast('event.updated', { event: updated });
        }
        catch { }
    ...
    remove: async (req, res) => {
        ...
        if (!ok)
            return res.status(404).json({ success: false, message: 'Event not found' });
        try {
            WSGateway.broadcast('event.deleted', { event_id });
        }
        catch { }
    ...
    view: async (req, res) => {
        ...
        try {
            WSGateway.broadcast('event.viewed', { event_id, by: req.user?.avitag ?? null });
        }
        catch { }
