...
export const GistController = {
    create: async (req, res) => {
        ...
        const gist = await GistService.create(req.user.avitag, req.user.account_id, profile_id, req.user.profileType || '', gist_text);
        ...
        try {
            ...
            if (inputs.length) {
                ...
                let idx = 0;
                for (const f of inputs) {
                    const uploaded = await uploadBuffer(f.data);
                    ...
                    idx += 1;
                    try {
                        WSGateway.broadcast('gist_media:created', { gist_id: gist.gist_id, media: saved });
                    }
                    catch { }
                }
            }
        ...
        try {
            const full = await GistService.findWithCountsAnyStatus?.(gist.gist_id);
            if (full)
    ...
    get: async (req, res) => {
        ...
        let full = await GistService.findWithCountsAnyStatus?.(id);
        if (!full)
            return res.status(404).json({ success: false, message: 'Gist not found' });
        // Populate profile object using avitag and account_id
    ...
    list: async (req, res) => {
        ...
        const viewer = req.user?.avitag ?? null;
        try {
            await Promise.all(data.map((g) => GistService.incrementView(g.gist_id, viewer)));
            WSGateway.broadcast('gist:viewed_batch', {
                gist_ids: data.map((g) => g.gist_id),
                by: viewer,
            });
            // Emit counts for each gist in batch
            const countsAll = await Promise.all(data.map((g) => GistService.getCountsFull(g.gist_id)));
            data.forEach((g, idx) => WSGateway.broadcast('counts:updated', { gist_id: g.gist_id, ...countsAll[idx] }));
        }
        catch { }
    ...
    update: async (req, res) => {
        ...
        const id = req.params.gist_id;
        const { gist_text } = req.body || {};
        const updated = await GistService.updateText(id, req.user.avitag, gist_text);
        if (!updated)
            return res
                .status(404)
    ...
    byUser: async (req, res) => {
        ...
        try {
            await Promise.all(data.map((g) => GistService.incrementView(g.gist_id, viewer)));
            WSGateway.broadcast('gist:viewed_batch', { gist_ids: data.map((g) => g.gist_id), by: viewer });
            const countsAll = await Promise.all(data.map((g) => GistService.getCountsFull(g.gist_id)));
            data.forEach((g, idx) => WSGateway.broadcast('counts:updated', { gist_id: g.gist_id, ...countsAll[idx] }));
        }
        catch { }
