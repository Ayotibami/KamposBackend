...
export async function listPending(limit = 20, offset = 0) {
    const { rows } = await pool.query(`SELECT * FROM gist_reports WHERE status = 'PENDING' ORDER BY created_at ASC LIMIT $1 OFFSET $2`, [limit, offset]);
    return rows;
}
export async function getById(report_id) {
    const { rows } = await pool.query(`SELECT * FROM gist_reports WHERE report_id = $1`, [report_id]);
    return rows[0] ?? null;
}
export async function listByGist(gist_id) {
    const { rows } = await pool.query(`SELECT * FROM gist_reports WHERE gist_id = $1 ORDER BY created_at DESC`, [gist_id]);
    return rows;
}
export async function rejectReport(report_id, idiot_avitag) {
...
export async function acceptReportAndRejectGist(report_id, idiot_avitag) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
