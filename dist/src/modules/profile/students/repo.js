...
export async function create(p) {
    const { rows } = await pool.query(`INSERT INTO student_profiles (
...
export async function findByAvitag(avitag) {
    const { rows } = await pool.query(`SELECT * FROM student_profiles WHERE avitag = $1`, [avitag]);
    return rows[0] ?? null;
}
...
export async function update(avitag, account_id, updates) {
    ...
    const { rows } = await pool.query(`UPDATE student_profiles SET ${fields.join(', ')} WHERE avitag = $${i++} AND account_id = $${i} RETURNING *`, vals);
    return rows[0] ?? null;
}
export async function setVerified(avitag, verified) {
    const { rowCount } = await pool.query(`UPDATE student_profiles SET is_verified = $1, updated_at = NOW() WHERE avitag = $2`, [verified, avitag]);
    return (rowCount || 0) > 0;
}
