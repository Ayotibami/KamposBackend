...
exports.get = get;
exports.reorderMedia = reorderMedia;
const db_1 = require("../../config/db");
async function addMedia(params) {
    const { rows } = await db_1.pool.query(`INSERT INTO gist_media (gist_id, media_type, media_url, thumbnail_url, order_index, public_id)
...
async function listByGist(gist_id) {
    const { rows } = await db_1.pool.query(`SELECT * FROM gist_media WHERE gist_id = $1 ORDER BY order_index ASC`, [gist_id]);
    return rows;
}
async function updateMedia(media_id, updates) {
    const fields = [];
    const vals = [];
    let i = 1;
    ...
    vals.push(media_id);
    const { rows } = await db_1.pool.query(`UPDATE gist_media SET ${fields.join(', ')} WHERE media_id = $${i} RETURNING *`, vals);
    return rows[0] ?? null;
}
async function remove(media_id) {
    const { rowCount } = await db_1.pool.query(`DELETE FROM gist_media WHERE media_id = $1`, [media_id]);
    return (rowCount || 0) > 0;
}
async function get(media_id) {
    const { rows } = await db_1.pool.query(`SELECT * FROM gist_media WHERE media_id = $1`, [media_id]);
    return rows[0] ?? null;
}
async function reorderMedia(gist_id, media_ids) {
