...
exports.getCampusMajor = getCampusMajor;
exports.findByAvitag = findByAvitag;
exports.listByAccount = listByAccount;
const db_1 = require("../../config/db");
async function getCampusMajor(avitag) {
    // Currently only student_profiles define campus_tag/major_tag
    const { rows } = await db_1.pool.query(`SELECT campus_tag, major_tag FROM student_profiles WHERE avitag = $1 LIMIT 1`, [avitag]);
    const r = rows[0];
...
async function findByAvitag(avitag) {
    ...
    for (const c of checks) {
        const { rows } = await db_1.pool.query(c.sql, [avitag]);
        if (rows[0]) {
